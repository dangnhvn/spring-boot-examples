...
public class CartControllerTest extends BaseControllerTest {
    ...
    public void TestCheckout () {
        HttpEntity httpEntity = new HttpEntity<>(null, getDefaultHeader());
        ResponseEntity<Response> responseEntity = restTemplate.exchange(getUrl() + CHECKOUT_ENDPOINT, HttpMethod.GET, httpEntity, Response.class);
        Assertions.assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
        Assertions.assertNotNull(responseEntity.getBody());

        ObjectError error = responseEntity.getBody().getError();
        Assertions.assertEquals("Order creation is fail due to cart is empty", error.getMessage());
        Assertions.assertEquals(ErrorCode.CREATION_ERROR, error.getCode());
